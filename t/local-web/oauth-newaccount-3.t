...
test {
  ...
  return post ("$wd/session", {
    ...
    return post ("$wd/session/$sid/url", {
      url => qq<http://$host/start?copied_data_field=id:abcid&create_email_link=0&server=> . $server_type,
    })->then (sub {
      ...
      return post ("$wd/session/$sid/url", {
        url => qq<http://$host/info?with_linked=id&with_linked=email>,
      });
    })->then (sub {
      return post ("$wd/session/$sid/execute", {
...
} wait => $wait, n => 3, name => ['/oauth create_email_link', $server_type], timeout => 120*5;
